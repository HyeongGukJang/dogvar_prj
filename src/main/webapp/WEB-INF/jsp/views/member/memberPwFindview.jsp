					...
					<div class="IdFind">
						<h2>비밀번호찾기</h2>
							...
							<form action="memberIdFind" method="POST" style="display: inline;">
								<input type="hidden" name="sendGb" value="2" />
								<table border="1">
									<tr>
										<th>아이디</th>
										<td><input type="text" maxlength="20" name="id" value="${sendInfo.id}" style="width: 200px"/></td>
									</tr>
									<tr>
							...
							<form action="emailSend" method="POST" style="display: inline;">
								<input type="hidden" name="sendGb" value="2" />
								<input type="hidden" name="email" value="${member.emailId}@${member.emailDomain}" />
								<input type="hidden" name="var" value="${member.id}" />
								<input type="submit" id="emailSendBtn" value="이메일발송" ${member != null ? 'null' : 'disabled' } />
								<input type="button" onclick="location.href='loginview'" value="로그인 화면">
							</form>
